...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:saxon="http://saxon.sf.net/"
  xmlns:xsl="http://www.w3.org/1999/XSL/Transform" queryBinding="xslt2" defaultPhase="structure" >
  ...
  <!-- following pattern suggested by George Bina in schematron mailing list, 
       resolves the ordering problem, however doesn't stop when a phases fails, 
       carries on with remaining phases -->
  <phase id="context">
  ...
  <pattern id="p-context">
    <rule context="*:thead">
      <assert test="not(exists(*:row/*:entry/@spanname))">Use of the spanname attribute in a thead (<value-of select="saxon:path()"/>) is not allowed CALS-T5R1</assert>
    </rule>
    
    <rule context="*:tfoot">
      <assert test="not(exists(*:row/*:entry/@spanname))">Use of the spanname attribute in a tfoot (<value-of select="saxon:path()"/>) is not allowed CALS-T6R1</assert>
    </rule>
  ...
  <pattern id="p-structure">
    <rule context="*:entry" >
      <assert test="every $e in following-sibling::*:entry
               satisfies empty(functx:value-intersect(cals:entry-to-columns(.), cals:entry-to-columns($e)))">The <xsl:value-of select="saxon:path()"/> occupying column(s) (<xsl:value-of
                 select="string-join(for $i in cals:entry-to-columns(.) return xs:string($i), ', ')"/>) overlaps some other entry or entries: <xsl:value-of
                 select="string-join(for $e in following-sibling::*:entry return if (exists(functx:value-intersect(cals:entry-to-columns(.), cals:entry-to-columns($e))))
                    then concat(saxon:path($e), ' occupying (', string-join(for $i in cals:entry-to-columns($e) return xs:string($i), ', '), ')') else (), ', ')" /> CALS-T10R4C</assert>
      <assert test="empty(functx:value-intersect(cals:entry-to-columns(.), cals:overlap2(parent::*:row)))">Column(s) (<xsl:value-of 
        select="functx:value-intersect(cals:entry-to-columns(.), cals:overlap2(parent::*:row))"/>) are occupied by both <xsl:value-of 
      ...
      <assert test="if (exists(@namest) or exists(@colname)) then 
                      min(cals:entry-to-columns(.)) ge cals:get-default-col-pos(.)
    ...
    <rule context="*:row">
      ...
      <xsl:variable name="occupied" select="for $e in *:entry return cals:entry-to-columns($e)"
        as="xs:integer*" />
